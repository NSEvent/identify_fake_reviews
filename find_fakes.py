...
class Review:
    ...
    def __str__(self):
        return 'Title: {}\nBody: {}\nScore: {}\n'.format(
            self.title, self.body, self.score)
...
def main():
    ...
    for i in range(1, 6):
        ...
        # use BeautifulSoup to make html parse tree, grab all reviews on the
        # page
        soup = BeautifulSoup(req.content, 'html5lib')
        titles = soup.findAll(
            'h3', attrs={'class': 'no-format inline italic-bolder font-20 dark-grey'})
        bodies = soup.findAll(
            'p', attrs={'class': 'font-16 review-content margin-bottom-none line-height-25'})
    ...
    # can use partial sort to make this faster, especially when looking at
    # more reviews
